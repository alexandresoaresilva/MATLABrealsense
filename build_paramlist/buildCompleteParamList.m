function buildCompleteParamList(app)
    ...
    opt_supported = zeros(rsoptionscount, 1);
    
    
    app.OptTableComplete = struct;
    ...
    for a=1:app.deviceCount
        for i = 1:rsoptionscount
            ...
            [opt_supported(i), ~, app.err] = calllib('realsense', 'rs_device_supports_option',...
      ...
      %not
          ...
          paramOPTtab = [paramOPTtabCount, supportedConnectedCam,...
              ...
              num2cell(opt_default), num2cell(opt_max), num2cell(opt_step), num2cell(opt_supported)];
        ...
        [app.OptTableComplete(:).(field)] = deal(dummyOptTable); %saves
        
        %for debugging ivcam retrieval
        SaveOptTableComplete = app.OptTableComplete;
        save('OptTableComplete.mat','SaveOptTableComplete');
