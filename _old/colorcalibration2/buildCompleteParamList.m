function buildCompleteParamList(app)
    ...
    for a=1:app.deviceCount
        for i = 1:rsoptionscount
            ...
            if  optsupportedDummy == 1%only saves supported options for camera
      ...
      %10. opt_step = parameter step size (only changes for white balance)
         ...
         dummyOptTable=cell(1,length(paramOPTtab(1,:))); %one line of the options table, cell array
         ...
         for i = 1:length(paramOPTtab)%gets rid of empty rows
           if ~isempty(paramOPTtab{i,1})
            ...
            field = char(strcat(app.connctdShortCamNames{a},strcat('_',string(a)))); %cams name to be struc field
